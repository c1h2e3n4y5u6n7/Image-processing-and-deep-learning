...
orgImg=cv2.imread('./originImg/circle.png')
#去噪
...
circles = AHTforCircles(edge,
                        center_threhold_factor=3.33,score_threhold=50.0,min_center_dist=80.0,minRad=0.0,
                        maxRad=1e7*1.0,center_axis_scale=1.0,radius_scale=1.0,
                        halfWindow=2,max_circle_num=1)
# circles=AHTforCircles(edge)

final_img = drawCircles(circles,blurred)
# cv2.imshow('hough_circle',final_img)
# cv2.waitKey(0)
plt.imshow(final_img, cmap='PRGn_r')
plt.axis('off')
plt.show()